...
These maximal predictabilities will be used throughout this work and will also be extend to a notion of maximal cross predictability in \autoref{sec:entropyrate}.
...
\begin{figure*}[h!]
	...
	\begin{subfigure}[t]{0.3\textwidth}
		...
		\caption{}
	\end{subfigure}
	\caption{Three examples of graphs and networks. (a) is an undirected clique graph with four nodes. (b) is a directed Erdős–Rényi network with four labelled nodes and $p = 2/3$. (c) is a undirected Watts-Strogatz graph with $n=8, k=2$ and $\beta=0.25$, where rewired edges are coloured.}
...
% \ts{There is much more I could add here (e.g. compound words, dealing with unknown characters, the comparison of character level vs word level tokens, unsupervised learning of tokenization (very cool)) but I feel that these are not relevant and worth leaving out.}
